...
export default function Register() {
  ...
  return (
    <div className="auth-split" style={{ background: 'var(--bg)', color: 'var(--text-1)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse 60% 50% at 30% 50%, rgba(34,211,238,0.06) 0%, transparent 60%)' }} />

      <div className="auth-panel-form" style={{ position: 'relative', zIndex: 10, overflowY: 'auto', maxHeight: '100vh', width: '100%' }}>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ width: '100%', maxWidth: 480, margin: '0 auto' }}>
          <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 9, textDecoration: 'none', marginBottom: 40 }}>
            <div style={{ width: 32, height: 32, borderRadius: 9, background: 'linear-gradient(135deg, rgba(139,92,246,0.25), rgba(34,211,238,0.15))', border: '1px solid rgba(139,92,246,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <span style={{ color: '#A78BFA', fontWeight: 800, fontSize: 14 }}>N</span>
            </div>
            <span style={{ fontFamily: 'var(--font-serif)', fontSize: 20, color: '#E5E7EB' }}>Nexus</span>
          </Link>

          <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(28px, 4vw, 34px)', fontWeight: 400, letterSpacing: '-0.02em', marginBottom: 8, color: '#E5E7EB' }}>Create your account</h1>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <button type="submit" disabled={loading} className="btn btn-primary" style={{ padding: '14px', width: '100%', marginTop: 8, opacity: loading ? 0.7 : 1 }}>
              {loading
                ? <span style={{ width: 18, height: 18, border: '2px solid rgba(255,255,255,0.25)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.7s linear infinite', display: 'block', margin: '0 auto' }} />
